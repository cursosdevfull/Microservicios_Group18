...
import { KafkaConsumer, KafkaProducer, RabbimqConsumer } from "@core/services"
...
(async () => {
  try {
    ...
    consumer.consume(env.GROUP_ID, env.TOPIC_CONSUME, async (message) => {
      console.log("================================");
      console.log("Received message CL desde Kafka:", message);

      const producer = new KafkaProducer()
      await producer.send(env.TOPIC_DLQ, [message])
    })
...
/*     const consumer = new RabbimqConsumer()
    ...
    await consumer.consumer(env.QUEUE_NAME, (message) => {
      console.log("================================");
      console.log("Received message CL:", message);
    }); */
